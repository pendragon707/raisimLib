...
template <typename N> int sgn(N val) {
...
namespace raisim
{
  ...
  public:
    explicit ENVIRONMENT(const std::string &resourceDir, const Yaml::Node &cfg, bool visualizable, int env_id) : RaisimGymEnv(resourceDir, cfg), visualizable_(visualizable)
    {
      ...
      READ_YAML(double, yawRewardCoeff_, cfg["yawRewardCoeff"])
      ...
      base_idx = go1_->getBodyIdx("base");
      // head_idx =  go1_->getFrameIdxByName("head_joint");

      footVec_.push_back(go1_->getBodyIdx("FR_calf"));
      footVec_.push_back(go1_->getBodyIdx("FL_calf"));
      footVec_.push_back(go1_->getBodyIdx("RR_calf"));
      footVec_.push_back(go1_->getBodyIdx("RL_calf"));

      std::cout << "FOOT VEC" << std::endl;

      std::cout << go1_->getBodyIdx("FR_calf") << std::endl;
      std::cout << go1_->getBodyIdx("FL_calf") << std::endl;
      std::cout << go1_->getBodyIdx("RR_calf") << std::endl;
      std::cout << go1_->getBodyIdx("RL_calf") << std::endl;

      std::cout << footVec_[0] << " " << footVec_[1] << " " << footVec_[2] << " " << footVec_[3] << " " << std::endl;

      footFrame_.push_back(go1_->getFrameIdxByName("FR_foot_fixed"));
      footFrame_.push_back(go1_->getFrameIdxByName("FL_foot_fixed"));
      footFrame_.push_back(go1_->getFrameIdxByName("RR_foot_fixed"));
      footFrame_.push_back(go1_->getFrameIdxByName("RL_foot_fixed"));
      ...
      // to store history
      last_torque.resize(go1_->getGeneralizedForce().size());
    ...
    void sample_goals()
    {
      ...
      {
        // delta_max_speed = -0.5 + 0.15 * (Eigen::VectorXd::Random(1)[0] / 2 + 0.5); // 0.075 ~ 0.15 
        // delta_ang_speed = 0.0 * ((std::rand() % 2) * 2 - 1) * (0.6);               // 0. 

        delta_max_speed = -0.15 + 0.15 * (Eigen::VectorXd::Random(1)[0] / 2 + 0.5); // -0.05 ~ 0.1
        delta_ang_speed = -0.05 + 0.15 * (Eigen::VectorXd::Random(1)[0] / 2 + 0.5); // 0.075 ~ 0.15
      ...
      else
      {
        ...
        delta_max_speed = -0.15 + 0.25 * Eigen::VectorXd::Random(1)[0]; // -0.05 ~ 0.2
        delta_ang_speed = -0.05 + 0.1 * Eigen::VectorXd::Random(1)[0]; // 0.35 ~ 0.55
      ...
      if (not sampleCmds)
      {
        ...
        delta_max_speed = -0.15 + 0.15 * Eigen::VectorXd::Random(1)[0]; // -0.05 ~ 0.1
        delta_ang_speed = -0.05 + 0.05 * Eigen::VectorXd::Random(1)[0];          // 0.4 ~ 0.5        
      }

      max_speed = 0.1 + delta_max_speed;
    ...
    double compute_angular_reward()
    {
      ...
      if (isDown && bodyAngularVel_[2] > (ang_speed+0.1))
        angular_r -= adaptiveAngularVelRewardCoeff_  * (bodyAngularVel_[2] - ang_speed);
    ...
    float step(const Eigen::Ref<EigenVec> &action_vec) final
    {
      ...
      VecDyn current_torque = go1_->getGeneralizedForce();
	...
	footSlipReward_ *= 4.;
      ...
      double angularReward = compute_angular_reward();
      double yawReward = sgn(bodyOrientation_[2]) * pow(bodyOrientation_[2], 2);
      ...
      auto cumulative_reward = angularReward + workReward_ * workRewardCoeff_ + yawReward * yawRewardCoeff_ +
                               cost_coeff * (contactReward_ * contactRewardCoeff_ + deltaContactReward_ * deltaContactRewardCoeff_ + 
                                             deltaReleaseReward_ * deltaReleaseRewardCoeff_ + footClearenceReward_ * footClearenceRewardCoeff_ +
                                             contactDistReward_ * contactDistRewardCoeff_ + contactChangeReward_ * contactChangeRewardCoeff_ +
                                             sidewaysReward_ * sidewaysRewardCoeff_);
