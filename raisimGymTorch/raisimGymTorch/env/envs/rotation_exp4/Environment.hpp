  ...
  public:
    ...
    void sample_goals()
    {
      ...
      if (sample_goal_rand_num > 0.6)
      {
        ...
        //delta_ang_speed = -0.5 + 0.15 * ((std::rand() % 2) * 2 - 1) * (0.3);    // -0.6 - 0.3 
      ...
      {
        //delta_max_speed = -0.05 + 0.1 * Eigen::VectorXd::Random(1)[0]; // 
      ...
      {
        adaptiveForwardVelRewardCoeff_ = forwardVelRewardCoeff_;
        delta_max_speed = -0.05 + 0.05 * Eigen::VectorXd::Random(1)[0]; // 
...
//    }
      if (isTest) 
      {
	std::cout << "max_speed: " << max_speed << ", ang_speed: " << ang_speed << std::endl;
	std::cout << "speed_vec: " << speed_vec[0] << ", " << speed_vec[1] << std::endl;
      }
    }
    ...
    //   forwardReward = compute_forward_reward();
      ...
      double yawReward = pow(bodyOrientation_[2], 2);
      ...
      // contactDistReward_ = pow((find_max(swing_bin, grf) - find_min(swing_bin, grf)), 2); // это заставляет чередовать ноги симметрично
