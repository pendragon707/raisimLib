...
namespace raisim
{
  ...
  public:
    ...
    // forward_vec =  data.site_xmat[self._imu_site_id] @ jp.array([1.0, 0.0, 0.0]) xmat — это матрицы вращения относительно одного сочленения
    // up_vec = array([0, 0, -1]) - на руках

    // forward_vec - bodyOrientation_
    // up_vec - целевое положение робота imu

    // up_vec << 0.0, 0.0, -1.0;
    double _reward_orientation(const Eigen::VectorXd& forward_vec, const Eigen::VectorXd& up_vec) const 
    {
      double cos_dist = forward_vec.dot(up_vec);
      double normalized = 0.5 * cos_dist + 0.5;
      return normalized * normalized;
    }    

    // torso_height = data.site_xpos[self._imu_site_id][2]  xpos являются координатами тела в глобальной системе координат before mj_ste
    //  self._z_des = 0.55 - на руках    

    // double x = gc_[0];
    // double y = gc_[1];
    // double torso_height = (hm_) ? hm_->getHeight(x, y) : 0;
    double _reward_height(double torso_height, double _z_des = 0.55) const 
    {
        double height = std::min(torso_height, _z_des);
        double error = _z_des - height;
        return std::exp(-error / 1.0);
    }

    // feet_geom_names = ["RR", "RL"]
    // self._feet_geom_ids = [0, 0, 1, 1]

    // feet_geom_ids << 0.0, 0.0, 1.0, 1.0;
    bool _cost_contact(const Eigen::VectorXd grf_bin, const Eigen::VectorXd feet_geom_ids) const {
      if (grf_bin == feet_geom_ids) {
        return true;
      }
      return false;
    }
    ...
    std::vector<raisim::Vec<3>> default_pose;

    raisim::Vec<3> footPosition;
    go1_->getFramePosition(footFrame_[footIdx_i], footPosition);
    default_pose.push_back(footPosition);

    double _cost_pose(const Eigen::VectorXd& qpos) const {
      _joint_ids = jp.array([6, 7, 8, 9, 10, 11])
      _joint_pose = default_pose[_joint_ids]

      Eigen::VectorXd diff = qpos(_joint_ids) - _joint_pose;  // extract by indices
      return diff.array().square().sum();
    }
    
    // VecDyn current_torque = go1_->getGeneralizedForce();
    double _cost_torques(const Eigen::VectorXd& current_torque) const {
        return current_torque.squaredNorm();
    }
    
    double _cost_energy(const Eigen::VectorXd& action) const {
        return action.squaredNorm();
    }
